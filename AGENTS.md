...
```
교재: /Users/sia/Documents/SKALA/4주차/KDT Back-end개발_2.REST API와 Springboot_2607_03.pdf
최종 실습: 527~561p
```
...
의도한 것이면 `docs/05-decisions.md`에 번호가 있어야 하고, 없으면 추가한다.
실수면 고친다.
...
3. 실제 구멍이라면 명세를 벗어나서라도 고칠 가치가 있는지 판단하고,
   고치기로 했으면 `docs/05-decisions.md`에 결정을 추가한다.
